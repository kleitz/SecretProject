...
class Twitter
{
	...
	{
		return $this->request('friendships/destroy', 'POST', array('screen_name' => $username));
	}
	
	
	/**
	 * Direct messages a user on Twitter.
	 * @param  string
	 * @return stdClass  see https://dev.twitter.com/rest/reference/post/friendships/create
	 * @throws TwitterException
	 */
	public function directmessage($username, $text)
	{
		return $this->request('direct_messages/new', 'POST', array('text' => $text, 'screen_name' => $username));
	}
	 ...
	 * @throws TwitterException
	 */
	public function getdm($count)
	{
		return $this->request('direct_messages', 'GET', array('count' => $count));
	}
	
	public function getdmsince($count, $sid)
	{
		return $this->request('direct_messages', 'GET', array('count' => $count, 'since_id' => $sid));
	}
	 ...
	 * @throws TwitterException
	 */
	public function getTrends($id = 1)
	{
		return $this->cachedRequest('trends/place', array('id' => $id));
	}
	...
	/**
	 * Returns friends of a given user.
	 * @param  string
	 * @return stdClass  see https://dev.twitter.com/rest/reference/get/followers/ids
	 * @throws TwitterException
	 */
	public function loadFriends($username, $cursor = -1, $cacheExpiry = null)
	{
		return $this->cachedRequest('friends/ids', array(
			'screen_name' => $username,
			'cursor' => $cursor,
		), $cacheExpiry);
	}
